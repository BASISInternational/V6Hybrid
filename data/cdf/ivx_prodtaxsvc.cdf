...
rem --- Show TAX_SVC_CD description
	...
	if salesTax!<>null() then
		taxSvcCd$=cvs(callpoint!.getColumnData("IVX_PRODTAXSVC.TAX_SVC_CD"),2)
		if taxSvcCd$<>"" then
			success=0
...
[[IVX_PRODTAXSVC.TAX_SVC_CD.AVAL]]
		...
		if taxSvcCd$<>"" then
			...
			if success then
				if desc$<>"" then
					...
					callpoint!.setColumnData("IVX_PRODTAXSVC.TAX_SVC_CD","",1)
					callpoint!.setStatus("ABORT")
					break
				endif
			else
				rem --- AvaTax call error
				callpoint!.setColumnData("IVX_PRODTAXSVC.TAX_SVC_CD","connect error",1)
				callpoint!.setStatus("ABORT")
				break
			endif
		else
