...
rem --- Show TAX_SVC_CD description
	...
	if salesTax!<>null() then
		taxSvcCd$=cvs(callpoint!.getColumnData("IVX_CLASTAXSVC.TAX_SVC_CD"),2)
		if taxSvcCd$<>"" then
			success=0
...
[[IVX_CLASTAXSVC.ITEM_TYPE.AVAL]]
rem --- Read IVC_CLASCODE to get tax service code
	ivc_clascode=fnget_dev("IVC_CLASCODE")
	dim ivc_clascode$:fnget_tpl$("IVC_CLASCODE")
	found=0
	readrecord(ivc_clascode,key=firm_id$+callpoint!.getUserInput(),dom=*next)ivc_clascode$;found=1
	if !found
		callpoint!.setStatus("ABORT")
		break
	else
		callpoint!.setColumnData("IVX_CLASTAXSVC.TAX_SVC_CD",ivc_clascode.tax_svc_cd$,1)
	endif

[[IVX_CLASTAXSVC.TAX_SVC_CD.AVAL]]
		...
		if taxSvcCd$<>"" then
			...
			if success then
				if desc$<>"" then
					...
					callpoint!.setColumnData("IVX_CLASTAXSVC.TAX_SVC_CD","",1)
					callpoint!.setStatus("ABORT")
					break
				endif
			else
				rem --- AvaTax call error
				callpoint!.setColumnData("IVX_CLASTAXSVC.TAX_SVC_CD","connect error",1)
				callpoint!.setStatus("ABORT")
				break
			endif
		else
