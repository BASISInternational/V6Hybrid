...
rem
rem
rem V6demo --- modified to work for BASIS on Addon 6.0 Data
rem
rem
rem ----------------------------------------------------------------------------
    rem ' trace
...
rem --- Get the IN parameters used by the procedure
    ...
    prior_year$ = fnyear_yy21$(num(prior_year$))
...
rem --- Get salesperson code from salesperson's name
    slspsn_desc$=pad(slspsn_desc$,len(arm10f.v6_code_desc$)," ")
    slspsn_code$=""
    trip_key$=firm_id$+"F"
    ...
    while 1
        ...
        if arm10f.v6_code_desc$=slspsn_desc$ then
            slspsn_code$=arm10f.v6_slspsn_code$
            break
        endif
    wend
...
rem --- salesVec! (0)=prior year sales, (1)=current YTD sales
    ...
    while 1
        ...
        if sam03a.v6_product_type$<>productType$ then gosub productType_break
...
rem --- Build result set for this salesperson's sales by product type
    ...
    if productTypeMap!.size()>0 then
        ...
        while productTypeIter!.hasNext()
            ...
            ivm10a.v6_code_desc$=unspecified_prod_type$; rem --- Sales might be summarized by salesrep with no product type
            readrecord(ivm10a_dev,key=firm_id$+"A"+productType$,dom=*next)ivm10a$
                
            data! = rs!.getEmptyRecordData()
            data!.setFieldValue("PRODUCT_TYPE",productType$+" - "+ivm10a.v6_code_desc$)
...
productType_break: rem --- Product type break
    ...
    productType$=sam03a.v6_product_type$
    productTypeSales=0
    return
